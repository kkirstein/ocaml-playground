...
let rec recv_loop ~verbose recv send =
  ...
  | n ->
      let res = if is_perfect (int_of_string n) then "t:" ^ n else "f:" ^ n in
      lwt_verbose ~verbose ("Sending: " ^ res) >>= fun () ->
      Zmq_lwt.Socket.send send res >>= fun () -> recv_loop ~verbose recv send
...
let cmd =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "$(tname) checks whether a given number is perfect.\n\
        \        This means that all its factors sum up to itself. Communication\n\
        \        with the worker is done via ZMQ, enabling a asynchronous and \
         parallel\n\
        \        execution.";
      `P
