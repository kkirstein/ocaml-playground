...
let color_of_value value =
  if value >= 255 then Image.color_black
    ...
    Image.
      { r = 5 * (value mod 15); g = 32 * (value mod 7); b = 8 * (value mod 31) }
...
let mandelbrot width height center_x center_y pixel_size =
  ...
  and y_offset = center_y +. (0.5 *. pixel_size *. float_of_int height) in
  let pixel_fun x y =
    let x_val = (float_of_int x *. pixel_size) +. x_offset
    and y_val = (float_of_int y *. pixel_size) -. y_offset in
    pixel_value x_val y_val 255 2.0 |> color_of_value
  in
  Image.map pixel_fun mandel

(* generate mandelbrot set (parallel version) *)
let par_mandelbrot ~pool width height center_x center_y pixel_size =
  let mandel = Image.make width height
  and x_offset = center_x -. (0.5 *. pixel_size *. float_of_int width)
  and y_offset = center_y +. (0.5 *. pixel_size *. float_of_int height) in
  let pixel_fun x y =
    ...
    pixel_value x_val y_val 255 2.0 |> color_of_value
  in
  Image.par_map ~pool pixel_fun mandel
