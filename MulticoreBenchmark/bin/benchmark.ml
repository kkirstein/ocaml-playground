...
let bench () = 
  ...
  Printf.printf "fib(35) = %s (Elapsed time %.3fs)\n"
    (Z.to_string res_fib.result) res_fib.elapsed;
  Printf.printf "fib(1000) = %s (Elapsed time %.3fs)\n"
    (Z.to_string res_fib_2.result) res_fib_2.elapsed;
  print_newline ();

  print_endline "Prime numbers";
  print_endline "=============";
  let res_primes = time_it Tasks.Primes.find_primes prime_limit
  in
  Printf.printf "find_primes(%d): %d primes (Elapsed time %.3fs)\n"
    prime_limit (List.length res_primes.result) res_primes.elapsed;
  let res_primes_z = time_it Tasks.Primes.find_primes_z (Z.of_int prime_limit)
  in
  Printf.printf "find_primes_z(%d): %d primes (Elapsed time %.3fs)\n"
    prime_limit (List.length res_primes_z.result) res_primes_z.elapsed;
  let res_sieve = time_it Tasks.Primes.sieve prime_limit
  in
  Printf.printf "sieve(%d): %d primes (Elapsed time %.3fs)\n"
    prime_limit (List.length res_sieve.result) res_sieve.elapsed;
  let res_sieve_z = time_it Tasks.Primes.sieve_z (Z.of_int prime_limit)
  in
  Printf.printf "sieve_z(%d): %d primes (Elapsed time %.3fs)\n"
    prime_limit (List.length res_sieve_z.result) res_sieve_z.elapsed;
  print_newline ();
  ...
  let res_pn_3 = time_it (Tasks.Perfect_number.perfect_numbers_par2 pool) pn_limit
  in
  Printf.printf "perfect_numbers_par2(%d) = %s (Elapsed time %.3fs)\n"
    pn_limit (string_of_int_list res_pn_3.result) res_pn_3.elapsed;
  print_newline ();

  print_endline "Mandelbrot set";
  print_endline "==============";
  let res_mandel_1 = time_it (fun _ -> Tasks.Mandelbrot.mandelbrot 640 480 (-0.5) 0.0 (4.0/.640.)) () in
